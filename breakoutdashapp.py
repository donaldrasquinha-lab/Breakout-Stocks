...
def get_mapping():
    """Downloads Upstox Master and maps Tickers to Instrument Keys."""
...
if source == "Upstox":
    ...
    if token:
        try:
            ...
            profile_res = requests.get(f"{UPSTOX_BASE}/user/profile", headers=get_v2_headers(token), timeout=15)
...
st.markdown("Scan Nifty 500 for breakouts (Price > 20d High & Vol > 1.5x Avg).")
    ...
    for i, t in enumerate(tickers):
        ...
        if source == "Yahoo Finance":
            ...
            if key:
                ...
                continue # Skip if ticker not in Upstox master
...
if os.path.exists("breakout_results.csv"):
    try:
        ...
        st.error("Data file error. Run a new scan.")
else:
